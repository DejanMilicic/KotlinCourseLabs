...
open class SquareBoardImpl(override val width: Int) : SquareBoard {
    ...
    override fun getCellOrNull(i: Int, j: Int): Cell? {
        return cells.firstOrNull { it.i == i && it.j == j }
    }

    override fun getCell(i: Int, j: Int): Cell {
        return getCellOrNull(i, j) ?: throw IllegalArgumentException("Cell does not exist")
    }
...
class GameBoardImpl(size: Int) : SquareBoardImpl(size), GameBoard {
    ...
    override fun find(predicate: (String?) -> Boolean): Cell? {
        return getAllCells().firstOrNull { predicate(values[it]) }
    }
