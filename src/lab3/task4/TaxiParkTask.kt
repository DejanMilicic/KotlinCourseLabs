...
internal fun TaxiPark.findFaithfulPassengers(minTrips: Int): Set<Passenger> {
    return allPassengers.filter {
        passenger -> trips.count { it.passengers.contains(passenger) } >= minTrips
    }.toSet()
}
...
internal fun TaxiPark.findFrequentPassengers(driver: Driver): Set<Passenger> {
    val driverTrips = trips.filter { it.driver == driver }

    return driverTrips.flatMap { it.passengers }
        .groupingBy { it }
        .eachCount()
        .filter { it.value > 1}
        .keys
...
internal fun TaxiPark.findSmartPassengers(): Set<Passenger> {
    return allPassengers.filter { passenger ->
        val passengerTrips = trips.filter { it.passengers.contains(passenger) }
        val discountTrips = passengerTrips.count { it.discount != null }
        val totalTrips = passengerTrips.size
        discountTrips > totalTrips / 2.0
    }.toSet()
}
