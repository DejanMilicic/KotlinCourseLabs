...
class MovieDB(private val movies: List<Movie>) : MovieDBApi{
    ...
    override fun getAllMoviesByActor(actor: MovieActor): List<Movie> = movies.filter { it.actors.contains(actor) }
    ...
    override fun getBestRatedMovieByActor(actor: MovieActor): Movie? = getAllMoviesByActor(actor).maxByOrNull { it.rating }
    ...
    override fun getBestRatedMovies(numOfMovies: Int): List<Movie> = movies.sortedByDescending { it.rating }.take(numOfMovies)

    override fun getDirectorWithMostMoviesDirected(): MovieDirector = movies.groupingBy { it.director }.eachCount()
        .maxByOrNull { it.value }?.key ?: throw NoSuchElementException("No movies available")
